...
def index():
    ...
    if request.method == 'POST':
        contentorder = OrderContent(orderdate=form_i.orderdate.data, expirationdate=form_i.expirationdate.data,
                                ...
                                amount3=form_i.amount3.data, amount4=form_i.amount4.data,
                                amount5=form_i.amount5.data)
...
@app.route('/checkpurchaseorder/<id>/edit', methods=['GET','POST'])
def edit(id):
    all_purchaseorder = OrderContent.query.all()
    form_i = InsertOrder()
    if request.method == 'POST':
    ...
    return render_template('edit.html', id=id, form_i=form_i)
